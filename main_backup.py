...
EE_SERVICE_ACCOUNT = os.getenv("EE_SERVICE_ACCOUNT")  # e.g. svc@project.iam.gserviceaccount.com
EE_PRIVATE_KEY_JSON_PATH = os.getenv("EE_PRIVATE_KEY_JSON_PATH")  # e.g. /app/ee_key.json
...
# EE state placeholders – will be created AFTER ee.Initialize()
era5 = None
modis_lst = None
modis_ndvi = None
...
# =========================
def init_gee():
    """Initialize Earth Engine: service account if provided, else default creds."""
    global EE_READY
    try:
        # Use the service account JSON file
        service_account = "harara-service@south-sudan-heatwave.iam.gserviceaccount.com"
        key_file = "south-sudan-heatwave-583da500ae5f.json"
        
        if os.path.exists(key_file):
            credentials = ee.ServiceAccountCredentials(service_account, key_file)
            ee.Initialize(credentials)
            print("✅ Earth Engine initialized using service account")
        elif EE_SERVICE_ACCOUNT and EE_PRIVATE_KEY_JSON_PATH and os.path.exists(EE_PRIVATE_KEY_JSON_PATH):
            creds = ee.ServiceAccountCredentials(EE_SERVICE_ACCOUNT, EE_PRIVATE_KEY_JSON_PATH)
            ee.Initialize(creds)
            print("✅ Earth Engine initialized using environment variables")
        else:
            # uses cached user auth if available (run `earthengine authenticate` once on this machine)
            ee.Initialize()
            print("✅ Earth Engine initialized using user credentials")
        EE_READY = True
...
def fetch_features_for_town(town_name, geom, lon, lat, lookback_days=LOOKBACK_DAYS) -> pd.DataFrame:
    end = dt.datetime.now(ZoneInfo(TIMEZONE)).date() - dt.timedelta(days=1)   # last full day
    ...
    # NDVI (buffered window to improve availability)
...
def prepare_window(df_recent: pd.DataFrame) -> np.ndarray:
    ...
    arr_scaled = SCALER.transform(arr)  # 2D scale
    return arr_scaled.reshape(1, LOOKBACK_DAYS, len(FEATURE_COLS))  # back to 3D
...
def _ensure_ee_objects_ready():
    """Safeguard in case startup order ever changes."""
    global era5, modis_lst, modis_ndvi, towns
    if not EE_READY:
        init_gee()
...
def run_predictions() -> Dict:
    ...
    # compute centroids after EE init
...
def predict_run():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Earth Engine error: {e}. "
                            f"If this is the first run on this machine, run `earthengine authenticate` "
                            f"or set EE_SERVICE_ACCOUNT/EE_PRIVATE_KEY_JSON_PATH.")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/predictions/today")
def predictions_today():
    """Return the latest run for today (if any)."""
    ...
    with Session(engine) as sess:
        ...
        for r in rows:
            ...
            by_run.setdefault(key, {
                "run_ts": key, "start_date": str(r.start_date),
                "end_date": str(r.end_date), "predictions": []
            })
...
def viz_today_png():
    """Quick bar chart of today's latest probabilities (PNG)."""
    import matplotlib
    matplotlib.use("Agg")
...
def on_startup():
    ...
    # init Earth Engine and build collections/geometries
    init_gee()
    build_ee_objects()
    ...
    scheduler.add_job(scheduled_job, CronTrigger(hour=7, minute=0))
    scheduler.start()
